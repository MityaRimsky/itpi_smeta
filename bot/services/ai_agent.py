...
Верни JSON с полями:
- work_type: тип работ - используй СТАНДАРТНЫЕ термины для поиска в базе:
  * "топографическая съемка" - для любых топопланов, инженерно-топографических планов, топосъемки
  ...
  * "опорная сеть" - для геодезических сетей
- quantity: объем работ (число) или null если не указан
...
- territory_type: тип территории - одно из: "застроенная", "незастроенная", "промпредприятие" или null
- work_stage: этап работ - одно из: "полевые", "камеральные", "обе" или null (если не указано явно)
- has_underground_comms: нужна ли съемка подземных коммуникаций (true/false/null)
- has_pole_sketches: нужны ли эскизы опор ЛЭП/связи (true/false/null)
- use_computer: использование компьютерных технологий для камеральных работ (true/false/null)
- height_section: высота сечения рельефа (0.5, 1.0, 2.0 м) или null
- update_mode: режим обновления существующего плана (true/false/null)

ВАЖНО: 
1. Для work_type используй СТАНДАРТНЫЕ термины из списка выше!
2. "инженерно-топографический план" = "топографическая съемка"
3. "топоплан" = "топографическая съемка"
4. Если параметр явно не указан - верни null

Примеры:
- "Создание инженерно-топографического плана м-ба 1:500" → work_type: "топографическая съемка"
- "топоплан 92 га промпредприятие" → work_type: "топографическая съемка", territory_type: "промпредприятие"
- "камеральные работы по топоплану" → work_type: "топографическая съемка", work_stage: "камеральные"
- "съемка с подземными коммуникациями" → has_underground_comms: true"""
    ...
    def determine_coefficients(self, params: Dict, work_stage: str = 'field') -> Tuple[List[str], Dict]:
        ...
        use_computer = params.get('use_computer', True)  # По умолчанию используем компьютер
        
        # К1 - Коэффициент территории (прим. 4 к т. 9)
        if territory == 'промпредприятие':
            coefficients.append('TERRITORY_INDUSTRIAL')
            details['К1'] = {'value': 1.75, 'reason': 'Территория промпредприятия (прим. 4)'}
        elif territory == 'застроенная':
            # Для застроенной территории коэффициент зависит от плотности застройки
            coefficients.append('TERRITORY_BUILT_UP')
            details['К1'] = {'value': 1.0, 'reason': 'Застроенная территория'}
        else:
            details['К1'] = {'value': 1.0, 'reason': 'Незастроенная территория (базовый)'}
        
        # К2 - Съемка подземных коммуникаций (прим. 5 к т. 9)
        if has_underground:
            coefficients.append('UNDERGROUND_COMMS')
            details['К2'] = {'value': 1.30, 'reason': 'Съемка подземных коммуникаций (прим. 5)'}
        else:
            details['К2'] = {'value': 1.0, 'reason': 'Без подземных коммуникаций'}
        
        # К3 - Эскизы опор (прим. 5 к т. 9)
        if has_poles:
            coefficients.append('POLE_SKETCHES')
            details['К3'] = {'value': 1.0, 'reason': 'С эскизами опор (прим. 5)'}
        else:
            details['К3'] = {'value': 1.0, 'reason': 'Без эскизов опор'}
        ...
        if work_stage == 'office' and use_computer:
            ...
            details['К4'] = {'value': 1.20, 'reason': 'Компьютерные технологии (ОУ 15г, 15д)'}
        
        # Коэффициент обновления (если обновляем существующий план)
        if params.get('update_mode'):
            coefficients.append('UPDATE_MODE')
            details['Кобн'] = {'value': 0.50, 'reason': 'Обновление существующего плана'}
        
        logger.info(f"Определены коэффициенты для {work_stage}: {coefficients}")
    ...
    def determine_addons(self, params: Dict, base_cost: float) -> List[str]:
        ...
        Args:
            params: Параметры работ
            base_cost: Базовая стоимость для определения категории транспорта
        ...
        if base_cost <= 75000:
            addons.append('INTERNAL_TRANSPORT_T4_1_1')  # до 75 тыс - 8.75%
        elif base_cost <= 150000:
            addons.append('INTERNAL_TRANSPORT_T4_1_2')  # до 150 тыс - 7.5%
        elif base_cost <= 300000:
            addons.append('INTERNAL_TRANSPORT_T4_1_3')  # до 300 тыс - 6.25%
        else:
            addons.append('INTERNAL_TRANSPORT_T4_1_4')  # свыше 300 тыс - 5%
...
*Объем:* {calc['quantity']} {work.get('unit', calc.get('unit', ''))}
